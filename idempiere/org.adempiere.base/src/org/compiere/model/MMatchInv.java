...
public class MMatchInv extends X_M_MatchInv
{
	...
	//code added by sysnova createMactInvCostDetail method added and used in Doc_MatchInv.java
	public String createMactInvCostDetail(MAcctSchema as, MInvoiceLine invoiceLine)
	{
		...
		if (receipt.getMovementType().equals(MInOut.MOVEMENTTYPE_VendorReturns)){
			...
			tAmt = tAmt.negate(); //added by shyamal on 10-12-16 : when return then qty is deducted from MCost and also deduct Amt.
		}
		else
			tQty = tQty.add(getQty());

		// Set Total Amount and Total Quantity from Matched Invoice
		///return msg updated by shyamal
		if (!MCostDetail.createInvoice(as, getAD_Org_ID(),
			...
			return "Current cost Qty/Rate is already -ve or will become -ve after this transaction for product: " + invoiceLine.getProduct().getValue()+". Please check product cost.";
		else
			return null;
	}
