...
//		}
	...
	public String completeIt()
	{
		...
		/**
		 * sysnova added for check same invoice/invoiceLine order_id and c_inout_id with header
		 * date 22 Oct 2018
		 */
		...
		{
			MInvoiceLine line = lines[i];
			if (line.getC_OrderLine_ID() != 0 && c_order_id!=0)
			{
				MOrderLine oLine = new MOrderLine (getCtx(), line.getC_OrderLine_ID(), get_TrxName());
				if(oLine.getC_Order_ID()!=c_order_id){
					m_processMsg = "C_Order_ID should not different between MR/shipment header and Line." + line.getLine();
					return DOCSTATUS_Invalid;
				}
			}
			if(c_order_id==0 && line.getC_OrderLine_ID()!=0){
				c_order_id=line.getC_OrderLine().getC_Order_ID();
			}
			if (line.getM_InOutLine_ID() != 0 && c_inout_id!=0)
			{
				MInOutLine InOutLine = new MInOutLine (getCtx(), line.getM_InOutLine_ID(), get_TrxName());
				if(InOutLine.getM_InOut_ID()!=c_inout_id){
					m_processMsg = "c_inout_id should not different between Invoice header and Line." + line.getLine();
					return DOCSTATUS_Invalid;
				}
			}
			if(c_inout_id==0 && line.getM_InOutLine_ID()!=0){
				c_inout_id=line.getM_InOutLine().getM_InOut_ID();
			}
		...
		for (int i = 0; i < lines.length; i++)
		{
			...
			//	sysnova added for auto batchclose or not
			if (!isSOTrx() && isAutobatchclose && line.getM_Product_ID() != 0 && line.getM_InOutLine_ID() != 0
					&& line.getC_OrderLine_ID()!=0){
				MAcctSchema[] acctschemas = MAcctSchema.getClientAcctSchema(getCtx(), getAD_Client_ID());
				MAcctSchema as = acctschemas[0];
				ol = (MOrderLine)line.getC_OrderLine();
				int mOrder_ID=ol.getC_Order_ID();//rifat added for get pricelist_ID
				MOrder mOrde= new MOrder(getCtx(), ol.getC_Order_ID(), get_TrxName());
				if(as.getC_Currency_ID()!=mOrde.getC_Currency_ID()){
					isAutobatchclose=false;
				}
				MInOutLine receiptLine = new MInOutLine (getCtx(),line.getM_InOutLine_ID(), get_TrxName());
				if(!ol.isNoOtherCost()){
					isAutobatchclose=false;
				}
				if(receiptLine.isBatchclosing()){
					isAutobatchclose=false;
				}
			...
			//sysnova auto batch close end
			//			
		}	//	for all lines
		
		/****sysnova********SCC-0001******Add*********/
		for (int i = 0; i < lines.length && !isSOTrx(); i++)
		{
			MInvoiceLine line = lines[i];
			MAcctSchema[] acctschemas = MAcctSchema.getClientAcctSchema(getCtx(), getAD_Client_ID());
			MAcctSchema as = acctschemas[0];
			if (MAcctSchema.COSTINGMETHOD_StandardCosting.equals(as.getCostingMethod()) && isAutobatchclose && line.getM_Product_ID()>1 && line.getM_InOutLine_ID()>1){
				MInOutLine oline = new MInOutLine (getCtx(),line.getM_InOutLine_ID(), get_TrxName());
				ArrayList costElement = new ArrayList();
				MProduct product=new MProduct(getCtx(), line.getM_Product_ID(), get_TrxName());
				if(product.isAsset()){
					continue; // only inventory item has cost
				}
				costElement=getCurrentCost (product,0, 
						as, line.getAD_Org_ID(),as.getM_CostType_ID(), as.getCostingMethod(),
						line.getQtyEntered(), 0, true, get_TrxName());
				
				if(costElement!=null){//for other cost element
					for (int j = 0; j < costElement.size(); j++){
						
						Vector costElementData = (Vector) costElement.get(j);
						int M_CostElement_ID1 = Integer.parseInt(""+costElementData.get(0));
						MCostVariance mCostVariance=new MCostVariance (getCtx(), 0, get_TrxName());	
						int M_CostElement_ID=mCostVariance.getCost_Element_Variance(M_CostElement_ID1,as, get_TrxName());
						if(M_CostElement_ID<1){
							m_processMsg = "M_CostElementvariance_ID not found for M_CostElement_ID="+M_CostElement_ID1;
							return DocAction.STATUS_Invalid;
						}
						BigDecimal amt= (BigDecimal) costElementData.get(1);
						BigDecimal totalamt=amt.multiply(line.getQtyEntered());
						...
						MMatchInv[] matchInvoices = MMatchInv.getInvoiceLine(getCtx(), line.getC_InvoiceLine_ID(), get_TrxName());
						MMatchInv minv =matchInvoices [0];
						
						if(minv.get_ID()<1){
							m_processMsg = "No matching found during Batch closing";
							return DocAction.STATUS_Invalid;
						}
						String retValue=mCostVariance.createCostVariancedetail (getAD_Org_ID(),line.getM_Product_ID(),line.getM_AttributeSetInstance_ID(),
								minv.get_ID(),M_CostElement_ID,amt.negate(),line.getQtyEntered(),"M_MatchInv_ID",line.get_TrxName(),"N");
						if(retValue.compareTo("@OK@")!=0)
						{
							MCostElement cl=new MCostElement(Env.getCtx(), M_CostElement_ID, get_TrxName());
							m_processMsg = cl.getName()+" not Saved <br>"+retValue;
							return DocAction.STATUS_Invalid;
						}
						retValue=mCostVariance.Savecostvariance_average(get_TrxName(), getAD_Org_ID(),
								amt.negate(),line.getQtyEntered(), line.getM_AttributeSetInstance_ID(),
								M_CostElement_ID, line.getM_Product_ID());
						if(retValue.compareTo("@OK@")!=0){
							MCostElement cl=new MCostElement(Env.getCtx(), M_CostElement_ID, get_TrxName());
							m_processMsg = cl.getName()+" not Saved for Org_ID="+getAD_Org_ID()
									+ "<br>"+retValue;
							return DocAction.STATUS_Invalid;
						}
					}
				...
				costElement=null;
				costElement=getMaterialCurrentCost (product,0, 
						as, line.getAD_Org_ID(),as.getM_CostType_ID(), as.getCostingMethod(),
						line.getQtyEntered(), 0, true, get_TrxName());
				if(costElement!=null){//for Only Material cost element
					for (int j = 0; j < costElement.size(); j++){
						Vector costElementData = (Vector) costElement.get(j);
						int M_CostElement_ID1 = Integer.parseInt(""+costElementData.get(0));
						MCostVariance mCostVariance=new MCostVariance (getCtx(), 0, get_TrxName());	
						...
						BigDecimal amt= (BigDecimal) costElementData.get(1);
						BigDecimal varianceAmtPerunit=line.getPriceEntered().subtract(amt);
						BigDecimal varianceAmttotal=varianceAmtPerunit.multiply(line.getQtyEntered());
						
						if(varianceAmtPerunit.signum()!=0){
							//MCostVariance mCostVariance=new MCostVariance (getCtx(), 0, get_TrxName());	
							String retValue=mCostVariance.Savecostvariance_average(get_TrxName(), getAD_Org_ID(),
									varianceAmtPerunit,line.getQtyEntered(), line.getM_AttributeSetInstance_ID(),
									M_CostElement_ID, line.getM_Product_ID());
							if(retValue.compareTo("@OK@")!=0){
								MCostElement cl=new MCostElement(Env.getCtx(), M_CostElement_ID, get_TrxName());
								m_processMsg = cl.getName()+" not Saved for Org_ID="+getAD_Org_ID()
				...
				oline.setisBatchclosing(true);
				if(!oline.save())
				{
					String lastError = CLogger.retrieveErrorString("");
					m_processMsg="MR Line Not Updated during Invoice Creation."+lastError;
					return DocAction.STATUS_Invalid;
				}
				boolean commitok=updateQtyOnhand(oline);
				if(!commitok)
				{
					String lastError = CLogger.retrieveErrorString("");
					m_processMsg= "updateQtyOnhand Problem. " + product.getValue() + " - " + lastError+lastErrorMsg;
					return DocAction.STATUS_Invalid;
				}			
			}
		}
		
		/****sysnova********SCC-0001******End*********/
...
//		counter.setBPartner(counterBP);// was set on copyFrom
	...
	/*
	 * Method add by shyamal for inter & intra company purpose and only called in matchAllocation.java file
	 */
	public BigDecimal getAllocatedAmtForIntraInter(int m_allocation_id)
		{
			BigDecimal retValue = null;
			String sql = "SELECT SUM(currencyConvert(al.Amount+al.DiscountAmt+al.WriteOffAmt,"
					+ "ah.C_Currency_ID, i.C_Currency_ID,ah.DateTrx,COALESCE(i.C_ConversionType_ID,0), al.AD_Client_ID,al.AD_Org_ID)) "
				...
				+ " INNER JOIN C_AllocationHdr ah ON (al.C_AllocationHdr_ID=ah.C_AllocationHdr_ID)"
				+ " INNER JOIN C_Invoice i ON (al.C_Invoice_ID=i.C_Invoice_ID) "
				+ "WHERE al.C_Invoice_ID=? and al.C_AllocationHdr_ID="+m_allocation_id+""
				+ " AND ah.IsActive='Y' AND al.IsActive='Y'";
			PreparedStatement pstmt = null;
			ResultSet rs = null;
			try
		...
		//	log.fine("getAllocatedAmt - " + retValue);
			//	? ROUND(NVL(v_AllocatedAmt,0), 2);
			return retValue;
		}	//	getAllocatedAmtForIntraInter

	//Method Add by shyamal 
	public BigDecimal getOpenAmtST (boolean creditMemoAdjusted, Timestamp paymentDate)
	{
		if (isPaid())
			return Env.ZERO;
		//
	...
	/**
	 * @author Rifat@ Sysnova
	 * @Date 10 sept 2014
	 * @Cause: 
	 * This file is writing for send sms to customer when sales order is completed.
	 * if customer's mobile number is found then only sent sms.
	 * @mail susbect: SMS of agent regarding credit feed delivery from
	 * 
	 */

	//private final String URL = "jdbc:postgresql://192.168.152.56:5432/adempiere"; //Rifat Pc
	private final String URL = "jdbc:postgresql://172.16.253.1:5432/sms"; //JEnoki Server
	public int sendConfirmationSMS() throws SQLException{
		
		String mobile_number=" ";
		String type="SOC";
		MActivity activity= new MActivity(Env.getCtx(), getC_Activity_ID(), get_TrxName());
		BigDecimal getTotal=getGrandTotal().setScale(2, BigDecimal.ROUND_CEILING);
		
		StringBuffer sql1 = new StringBuffer("");
		sql1 = new StringBuffer("");
		sql1.append("SELECT bpl.Phone " +
		" FROM " +
		" C_BPartner_Location bpl " +
		" WHERE bpl.C_BPartner_ID=" +getC_BPartner_ID()+
		" AND bpl.IsActive='Y' AND bpl.Phone IS NOT NULL AND bpl.Phone LIKE '+8801%' AND char_length(bpl.Phone) = 14 " +
		" AND bpl.AD_Client_ID = "+getAD_Client_ID()+" order by C_BPartner_Location_ID desc");
		...
		String uomName="";
		MInvoiceLine[] oLines = getLines();//(true, null);
		for (int i = 0; i < oLines.length; i++) {
			MInvoiceLine oLine = oLines[i];
			totalQty=totalQty.add(oLine.getQtyInvoiced());
			MUOM uom= new MUOM (Env.getCtx(), oLine.getC_UOM_ID(), get_TrxName());
			uomName=uom.getX12DE355();
		}
		totalQty=totalQty.setScale(2, BigDecimal.ROUND_CEILING);
		String sms_text="Thank you for your "+activity.getName()+" order("+getDocumentNo()+") of Tk. "+getTotal+" @ Kazi Food.";
		
		java.sql.Connection myRemote = java.sql.DriverManager.getConnection(URL,"adempiere","adempiere");
		int table_ID=-1;
		int totalInsertno=-1;
		...
		try
		{
			/*String sql = "SELECT nextval('outbox_id_seq')";
			
			java.sql.Statement stmt = myRemote.createStatement();
			...
			rset=null; */
			if(myRemote != null) {
				st = myRemote.createStatement();
			String	sql = "INSERT INTO outbox (\"number\",\"text\",\"insertdate\",\"type\") " +
						"VALUES ('"+mobile_number+"','"+sms_text+"',TO_DATE('"+getDateAcct()+"','YYYY-MM-DD'),'"+type+"')";
				if(st != null)
					totalInsertno = st.executeUpdate(sql); 
			}
		}
		catch (Exception e) {
	...
	public static ArrayList getMaterialCurrentCost (MProduct product,int M_ASI_ID,
			MAcctSchema as, int Org_ID, int M_CostType_ID,
			String costingMethod, BigDecimal qty, int C_OrderLine_ID,
			boolean zeroCostsOK, String trxName)
		{
			...
			String sql = "SELECT"
				...
				+ " AND (c.M_AttributeSetInstance_ID=? OR c.M_AttributeSetInstance_ID=0)"	//	#4
				+ " AND c.M_CostType_ID=? AND c.C_AcctSchema_ID=?"	//	#5/6
				+ " AND (ce.CostingMethod IS not NULL) and CostingMethod='S' and costelementtype='M' "
				+ " AND ce.isactive='Y' "
				+ "GROUP BY ce.CostElementType, ce.CostingMethod, c.Percent, c.M_CostElement_ID";
			PreparedStatement pstmt = null;
			ResultSet rs = null;
	...
	String lastErrorMsg ="";
	public boolean updateQtyOnhand(MInOutLine line1){

		boolean commitok=true;
		try {
			//Vector line1=(Vector)infodata.get(lotPickcomboID);
			int M_Product_ID=line1.getM_Product_ID();
			MProduct product=new MProduct(Env.getCtx(), M_Product_ID, get_TrxName());
			int M_InOutLine_ID=line1.get_ID();
			MInOutLine sLine=new MInOutLine(Env.getCtx(), M_InOutLine_ID, get_TrxName());
			MInOut m= new MInOut(Env.getCtx(), sLine.getM_InOut_ID(), get_TrxName());
			BigDecimal isQty=line1.getQtyEntered();
			
			Timestamp dateMPolicy= null;
			MStorageOnHand[] storages = null;
			
			if(isQty.compareTo(Env.ZERO) > 0){
				storages = MStorageOnHand.getWarehouse(Env.getCtx(), 0,
			...
			if (!MStorageOnHand.removeNotClosingQty(Env.getCtx(), m.getM_Warehouse_ID(),
					sLine.getM_Locator_ID(),
					sLine.getM_Product_ID(),
					sLine.getM_AttributeSetInstance_ID(),
					isQty,dateMPolicy,get_TrxName()))
			{
				commitok=false;;
