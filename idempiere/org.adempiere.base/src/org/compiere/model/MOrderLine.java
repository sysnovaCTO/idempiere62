...
public class MOrderLine extends X_C_OrderLine
{
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		/**
		 * sysnova
		 * When user select Product, user should select other cost Or No Cost
		 * 
		 */
		
		if (getM_Product_ID() == 0){
			setIsClearingExpence(false);
			setIsDutyTaxes(false);
			setIsFinancialCharge(false);
			setIsFreightInward(false);
			setIsNoOtherCost(true);
		}
		if(!m_IsSOTrx){
			if (getM_Product_ID() != 0){
				MProduct product = getProduct();
				if(product.getProductType().equals("I"))
				{
					if(!isClearingExpence() && !isDutyTaxes() && !isFinancialCharge() && !isFreightInward()
							&& !isNoOtherCost()){
						log.saveError("FillMandatory", Msg.translate(getCtx(), "Select Any Additional Cost (Cost Element)"));
						return false;
					}
					if((isClearingExpence() || isDutyTaxes() || isFinancialCharge() || isFreightInward())
							&& isNoOtherCost()){
						log.saveError("FillMandatory", Msg.translate(getCtx(), "UnSelect NoOtherCost"));
						return false;
					}
				}
				else
		...
		/**
		 * sysnova added activity, it is mandatory during auto invoice
		 *
		 */
		if(getC_Activity_ID()<1){
			MOrder order=new MOrder(getCtx(),getC_Order_ID(), get_TrxName());
			setC_Activity_ID(order.getC_Activity_ID());
		}
		
		//sysnova End 21 April 2018
