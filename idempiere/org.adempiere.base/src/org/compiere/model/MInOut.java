...
public class MInOut extends X_M_InOut implements DocAction
{
	...
	public String completeIt()
	{
		...
		/**
		 * sysnova added for check same MR/ShipmentLine order_id with header
		...
		for (int lineIndex = 0; lineIndex < lines.length; lineIndex++)
		{
			...
			if (sLine.getC_OrderLine_ID() != 0 && c_order_id!=0)
			{
				MOrderLine oLine = new MOrderLine (getCtx(), sLine.getC_OrderLine_ID(), get_TrxName());
				if(oLine.getC_Order_ID()!=c_order_id){
					m_processMsg = "C_Order_ID should not different between MR/shipment header and Line." + sLine.getLine();
		...
		for (int lineIndex = 0; lineIndex < lines.length; lineIndex++)
		{
			...
			{
				/**
				 * Rifat Added For stop reverse when Batch Is closed 
				 */
				if (!isSOTrx()&& isReversal()){
					if(sLine.isBatchclosing()){
						m_processMsg = "Batch Already Closed. Line: " + sLine.getLine();
						return DOCSTATUS_Invalid;
					}
				}
				MDocType dt = MDocType.get(getCtx(), getC_DocType_ID());
				if(!dt.getName().equals("MM Returns")){
					if(product != null && product.get_ID()>1){
						BigDecimal cost=getCurrentcost(sLine);
						if(cost.signum()!=1){
							m_processMsg = "Product Standard Cost (Cost Element=Material) Not found. MR Line No: "+ sLine.getLine()+" [" + product.getValue() + "]";
							return DocAction.STATUS_Invalid;
						}
						if(!product.isItem() && !product.isAsset()){
							m_processMsg = "Service Product Not allow during MR Complete. MR Line No: "+ sLine.getLine()+" [" + product.getValue() + "]";
							return DocAction.STATUS_Invalid;
						}
						if (sLine.getC_OrderLine_ID() == 0){
							m_processMsg = "Could not Complete MR. C_OrderLine_ID not found for MR Line No: "+ sLine.getLine()+"[" + product.getValue() + "]";
							return DocAction.STATUS_Invalid;
						}
						if(product.isAsset()){
							isAutoInv=false;
						}
						if (!isSOTrx() && isAutoInv){
							MAcctSchema[] acctschemas = MAcctSchema.getClientAcctSchema(getCtx(), getAD_Client_ID());
							MAcctSchema as = acctschemas[0];
							MOrderLine ol = (MOrderLine)sLine.getC_OrderLine();
						...
						sLine.setisBatchclosing(true);
						if (!sLine.save())
						{
							m_processMsg = CLogger.retrieveErrorString("Could not save isBatchclosing. lineNo: "+((lineIndex*10)+1));
							return DocAction.STATUS_Invalid;
						}
					}
				}
						...
						for (int j = 0; j < mas.length; j++)
						{
							...
							//// sysnova added: Start	
								...
								if (!MStorageOnHand.add(getCtx(), getM_Warehouse_ID(),
									...
									{
										String lastError = CLogger.retrieveErrorString("");
										/****Rifat********CNV-0001******blocled 4.1*********/
										//m_processMsg = "Cannot correct Inventory OnHand (MA) [" + product.getValue() + "] - " + lastError;
										/****Rifat********CNV-0001******End*********/
										/****Rifat********CNV-0001******Added*********/
										m_processMsg = "Inventory is already negative or will be negative after this transaction";
										/****Rifat********CNV-0001******End*********/
										return DocAction.STATUS_Invalid;
									}
							}
					...
					if (mtrx == null)
					{
						...
						////  sysnova added: END
								...
								Qty,dateMPolicy,get_TrxName()))
							{
								...
								m_processMsg = m_processMsg+" Inventory is already negative or will be negative after this transaction";
				...
				if(isSOTrx() && sLine.getM_Product_ID()>1){
					
					MAcctSchema[] acctschemas = MAcctSchema.getClientAcctSchema(getCtx(), getAD_Client_ID());
					MAcctSchema as = acctschemas[0];
					MLocator locator = new MLocator (getCtx(), sLine.getM_Locator_ID(), get_TrxName());
					if (MAcctSchema.COSTINGMETHOD_StandardCosting.equals(as.getCostingMethod())){
						MCostVariance mCostVariance=new MCostVariance (Env.getCtx(), 0, get_TrxName());	
						ArrayList VarianceData=null;
						if(dt.getName().equals("MM Returns")){
							VarianceData=mCostVariance.findVarianceForMMReturns(locator.getAD_Org_ID(), sLine.getM_Product_ID(),sLine.getM_AttributeSetInstance_ID(), get_TrxName());
						}
						else{
							VarianceData=mCostVariance.findVariance(locator.getAD_Org_ID(), sLine.getM_Product_ID(),sLine.getM_AttributeSetInstance_ID(), get_TrxName());
						}
						for (int ii = 0; ii < VarianceData.size(); ii++){
							...
							String retValue=mCostVariance.createCostVariancedetail (locator.getAD_Org_ID(),sLine.getM_Product_ID(),sLine.getM_AttributeSetInstance_ID(),
									sLine.get_ID(),M_CostElement_ID,currentcostprice,Qty,"M_InOutLine_ID",sLine.get_TrxName(),"Y");
							...
							retValue="";
							//locator
							retValue=mCostVariance.Savecostvariance_average(get_TrxName(), locator.getAD_Org_ID(),
									currentcostprice,Qty, sLine.getM_AttributeSetInstance_ID(),
									M_CostElement_ID, sLine.getM_Product_ID());
							if(retValue.compareTo("@OK@")!=0){
								MCostElement cl=new MCostElement(Env.getCtx(), M_CostElement_ID, get_TrxName());
								m_processMsg = cl.getName()+" not Saved for Org_ID="+locator.getAD_Org_ID()
										+ "<br>"+retValue;
								return DocAction.STATUS_Invalid;
							}
	...
	/// sysnova added: start
	public BigDecimal getCurrentcost(MInOutLine line) {
		//
		int M_CostElement_ID = DB.getSQLValue(get_TrxName(),
				"SELECT M_CostElement_ID FROM M_CostElement WHERE Name='Material' and AD_Client_ID="+getAD_Client_ID());
		if(M_CostElement_ID<1) {
			throw new AdempiereException(Msg.getMsg(getCtx(), "Cost Element Material Not Found") );
		}
		BigDecimal cost=Env.ZERO;
		String sql = "select  coalesce((currentcostprice+currentcostpricell),0) from M_cost"
				+ " where  ad_client_id = "+line.getAD_Client_ID()
				+ " and ad_org_id="+line.getAD_Org_ID()
				+ " and m_product_id= "+line.getM_Product_ID()
				+ " and M_CostElement_ID="+M_CostElement_ID;

		try {
			PreparedStatement pstmt = DB.prepareStatement(sql, line.get_TrxName());
