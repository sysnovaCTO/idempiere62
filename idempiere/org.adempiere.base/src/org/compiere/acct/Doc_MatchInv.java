...
//		MMatchInv matchInv = (MMatchInv)getPO();
		...
		**/
		...
		/* sysnova *******Add********SL-0001**********Add******************/
		if (dr.getC_Currency_ID() != cr.getC_Currency_ID()){
			
			MInvoice invoice = m_invoiceLine.getParent();
			BigDecimal conversionRate=invoice.getConversionRate();
			if(conversionRate.signum()!=1){
				p_Error = "conversion Rate not found";
				return null;
			}
			BigDecimal invAmt = conversionRate.multiply(m_invoiceLine.getLineNetAmt());
			
			BigDecimal LineAmt = MConversionRate.convert(getCtx(), m_invoiceLine.getLineNetAmt(),
					invoice.getC_Currency_ID(), as.getC_Currency_ID(),
					invoice.getDateAcct(), invoice.getC_ConversionType_ID(),
					invoice.getAD_Client_ID(), invoice.getAD_Org_ID());
			BigDecimal rateVariance=invAmt.subtract(LineAmt).negate();;
			processRateVariance(as, fact, rateVariance);
			ipv=ipv.subtract(rateVariance);
		}
		...
		/*********Add*******DM-0001************Add****************/
		/** sysnova
		 * Added auto batch closing journal without Invoice Price Variance
		 */
		if (MAcctSchema.COSTINGMETHOD_StandardCosting.equals(as.getCostingMethod()))
		{
			MMatchInv line=m_matchInv;
			...
			ArrayList VarianceData=mCostVariance.findVarianceDetails(line.getAD_Org_ID(), line.getM_Product_ID(),line.getM_AttributeSetInstance_ID(),
					line.get_ID(),"M_MatchInv_ID", getTrxName());
			for (int ii = 0; ii < VarianceData.size(); ii++){
				Vector vLine=(Vector)VarianceData.get(ii);
				int T_costVariance_ID  = ((Integer)vLine.get(0)).intValue();
				if(T_costVariance_ID<1){
					continue;
				}
				...
				int C_ValidCombination_ID=getC_ValidCombinationVariance_acct(M_CostElement_ID, as, getTrxName());
				int C_ValidCombination_IDdr=get_Cost_Clearing_acct(M_CostElement_ID, as, getTrxName());
				if(C_ValidCombination_ID>0 && C_ValidCombination_IDdr > 0){
					BigDecimal postAmt=Env.ZERO;
					MAccount acctCR = MAccount.get (as.getCtx(), C_ValidCombination_IDdr);
					MAccount acctDR = MAccount.get (as.getCtx(), C_ValidCombination_ID);
					FactLine drr =null;
					FactLine crr =null;
					postAmt=currentcostprice.multiply(line.getQty());
					if(postAmt.signum()>0)
						drr = fact.createLine(null, acctDR,as.getC_Currency_ID(), postAmt, null);
					else{
						drr = fact.createLine(null, acctDR,as.getC_Currency_ID(),  null,postAmt.abs());
					}
						//
						if (drr == null)
						{
						...
						drr.setM_Product_ID(line.getM_Product_ID());
						//drr.setM_Locator_ID(line.getM_LocatorTo_ID());
						drr.setLocationFromBPartner(getC_BPartner_Location_ID(), true);   // from Loc
						//drr.setLocationFromLocator(line.getM_Locator_ID(), false);   // to Loc
						drr.setQty(line.getQty());
						if(postAmt.signum()>0)
							crr = fact.createLine(null,acctCR,as.getC_Currency_ID(), null, postAmt);
						else 
							crr = fact.createLine(null,acctCR,as.getC_Currency_ID(), postAmt.abs(), null);
						//
						if (crr == null)
						{
						...
						crr.setM_Product_ID(line.getM_Product_ID());
						//crr.setM_Locator_ID(line.getM_Locator_ID());
						crr.setLocationFromBPartner(getC_BPartner_Location_ID(), true);   //  from Loc
						//crr.setLocationFromLocator(line.getM_Locator_ID(), false);   //  to Loc
						crr.setQty(line.getQty());
				}
				else{
					p_Error = "CostVariance_acct Or CostClearing_acct Not Found For M_CostElement_ID "+M_CostElement_ID+" CR not created: " + line;
					log.log(Level.WARNING, p_Error);
					return null;
				}
			}
		...
		/* sysnova *********End******DM-0001*************End***************/
		
		//code added by sysnova	
		MCostDetail cd = MCostDetail.get(getCtx(), "C_InvoiceLine_ID = ?", m_invoiceLine.getC_InvoiceLine_ID(),
				m_invoiceLine.getM_AttributeSetInstance_ID(), as.getC_AcctSchema_ID(), m_invoiceLine.get_TrxName());
		if (cd == null)
		{
			MMatchInv matchInv = (MMatchInv) getPO();
			error = matchInv.createMactInvCostDetail(as, m_invoiceLine);
			if (error != null && error.trim().length() > 0)
			{
				p_Error = error;
				return null;
	...
	/********Add********SL-0001**********Add******************/
	protected String updateinvoicePriceCostsElement(MAcctSchema as,
			BigDecimal ipv) {
		MProduct product= new MProduct (getCtx(),getM_Product_ID() , getTrxName());
		MCostElement[] ces = MCostElement.getCostingMethods(product);
		MCostElement ce = null;
		int T_costVariance_ID=0;
		...
		if (ce == null)
		{
			...
			return " No Standard Costing in System";
		}
		int m_costelement_id =0;
		try {
			m_costelement_id = DB.getSQLValue(null,
					"SELECT m_costelementvariance_id from M_CostElement_Acct where M_CostElement_ID="+ce.get_ID());
		} catch (Exception e) {
			// TODO: handle exception
		}
		if(m_costelement_id<1){
			s_log.fine("No Invoice Price Variance Element in System");
			return " No Invoice Price Variance Element in System";
		}
		BigDecimal costprice=Env.ZERO;
		BigDecimal currentqty=Env.ZERO;
		String sql = (
				  " select T_costVariance_ID,currentcostprice+currentcostpricell,currentqty,m_costtype_id"
				+ " from T_costVariance"
				+ " where ad_client_id="+m_invoiceLine.getAD_Client_ID()
				+ " and ad_org_id = "+m_invoiceLine.getAD_Org_ID()
				+ " and c_acctschema_id ="+as.get_ID()
				+ " and m_costelement_id ="+m_costelement_id
				+ " and m_costtype_id  = "+as.getM_CostType_ID()
				+ " and m_product_id  = "+m_invoiceLine.getM_Product_ID()
				+ " and m_attributesetinstance_id = "+m_invoiceLine.getM_AttributeSetInstance_ID()
				);
			...
			try
			{
				...
				while (rs.next ())
				{
					T_costVariance_ID=rs.getInt(1);
					costprice=rs.getBigDecimal(2);
					currentqty=rs.getBigDecimal(3);
		...
		BigDecimal newCurrentqty=currentqty.add(m_invoiceLine.getQtyEntered());
		BigDecimal totalcostprice=costprice.add(ipv);
		BigDecimal currentcostprice=totalcostprice.divide(newCurrentqty, 12, BigDecimal.ROUND_HALF_UP);
		if(T_costVariance_ID<1){ // no record.
			
			X_T_CostVariance cv=new X_T_CostVariance (getCtx(),T_costVariance_ID, getTrxName());			
			cv.setAD_Org_ID(m_invoiceLine.getAD_Org_ID());
			cv.setC_AcctSchema_ID(as.get_ID());
			cv.setM_CostType_ID(as.getM_CostType_ID());
			cv.setM_CostElement_ID(m_costelement_id);
			cv.setM_Product_ID(m_invoiceLine.getM_Product_ID());
			cv.setM_AttributeSetInstance_ID(m_invoiceLine.getM_AttributeSetInstance_ID());
			cv.setCurrentQty(newCurrentqty);
			cv.setCurrentCostPrice(currentcostprice);
			cv.setCurrentCostPriceLL(Env.ZERO);
			cv.setCumulatedQty(newCurrentqty);
			cv.setCumulatedAmt(totalcostprice);
			  
			if(!cv.save()){
				return " Invoice Price Variance not Saved";
			}
		}
		return "";
	}
	
	protected void processRateVariance(MAcctSchema as, Fact fact,
			BigDecimal ipv) {
		if (ipv.signum() == 0) return;
		
		int m_costelement_id =0;
		try {
			m_costelement_id = DB.getSQLValue(getTrxName(),
					"SELECT M_CostElement_ID from M_CostElement where Name='Exchange Rate Variance' and AD_Client_ID="+as.getAD_Client_ID());
		} catch (Exception e) {
			// TODO: handle exception
			throw new RuntimeException("No Exchange Rate Cost Element in System");
		}
		if(m_costelement_id<1){
			throw new RuntimeException("No Exchange Rate Cost Element in System");
		}
		
		int costclearing_acct_id =0;
		try {
			costclearing_acct_id = DB.getSQLValue(getTrxName(),
					"select costclearing_acct from M_CostElement_Acct where M_CostElement_ID="+m_costelement_id);
		} catch (Exception e) {
			// TODO: handle exception
			throw new RuntimeException("Costclearing account not found for Exchange Rate Element in System");
		}
		if(costclearing_acct_id<1){
			throw new RuntimeException("Costclearing account not found for Exchange Rate Element in System");
		}
		MAccount account=new MAccount(getCtx(), costclearing_acct_id, getTrxName());
		...
		try {
			...
			if (!MCostDetail.createMatchInvoice(as, m_invoiceLine.getAD_Org_ID(),
					m_invoiceLine.getM_Product_ID(), m_invoiceLine.getM_AttributeSetInstance_ID(),
					matchInv.getM_MatchInv_ID(), m_costelement_id,
					ipv, BigDecimal.ZERO, "Exchange Rate Variance", getTrxName())) {
				throw new RuntimeException("Failed to create cost detail record.");
			}				
		} catch (SQLException e) {
			throw new RuntimeException(e.getLocalizedMessage(), e);
	...
	public static int getC_ValidCombinationVariance_acct (int costElementData_ID,MAcctSchema as, String trxName )
	{
		...
		String sql = " SELECT CostVariance_acct FROM  M_CostElement_Acct "+
				" WHERE  m_costelementvariance_id=? AND C_AcctSchema_ID=? AND AD_Client_ID=? and isActive='Y'";
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try
		{
	...
	public static int get_Cost_Clearing_acct (int costElementData_ID,MAcctSchema as, String trxName )
	{
		int CostClearing_acct=0;
